...
app.post('/validate', function (req, res) {
    ...
    pool.query('SELECT * FROM restart where uid = $1',[req.body.zuid] , function (err, result) {
        if(err) {
            ...
            if(req.body && req.body.zuid && req.body.uemail) {
                ...
                if(result.rows.length === 0) {
                    ...
                    pool.query("INSERT INTO restart VALUES ($1, $2 , ' ', ' ', ' ', ' ', ' ', ' ')", 
                        [req.body.zuid, req.body.uemail ], function (err) 
                        {
                            if(err) {
...
app.post('/24x7update', function (req, res) {
    ...
    if(req.body.zuid.equals("113386512894341810206")) {
        runkey = "ME";
    }

    pool.query('UPDATE restart SET gitcommit = $1, gitusername = $2,dinoisses = $3, runkey = $4 WHERE uid = $5',
        [ req.body.Commit, req.body.Username, req.body.dinoisses, runkey ,req.body.zuid ], function (err, result)
        {
            if(err) {
                res.status(500).send(err.toString());
            }
...
app.post('/24x7stop', function (req, res) {
    ...
    pool.query('UPDATE restart SET runkey = $1 WHERE uid = $2',[ runkey, req.body.zuid ], function (err, result) {
        if(err) {
...
{
    pool.query("SELECT * FROM restart WHERE runkey ='START'", function (err, result) {
        if(err) {
            console.log(err);
        } else {
    ...
    setInterval(function () {
        pool.query("SELECT * FROM restart WHERE runkey ='ME'", function (err, result) {
            if(err) {
                console.log(err);
            } else {
                for(index = 0 ; index < result.rows.length; index++)
